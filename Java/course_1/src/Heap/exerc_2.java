...
//Implement a function named streamMax that processes a stream of integers and returns the maximum number encountered so
//far for each input number.
//This function should take in an array of integers and return a list of integers.
//Given an array of integers, your function should iterate over the array and for each number, it should find the maximum number
//in the array up until that point, including the current number.
//
//The streamMax function has the following signature:
//public static List<Integer> streamMax(int[] nums)
//
//It accepts one parameter: nums (an array of integers): An array of integers  for each i from 0 to nums.length - 1.
//
//The function should return: A list of integers: The i-th integer in the list should be the maximum number among
//the first i numbers in the input array nums (1-indexed).
//
//
...
public class exerc_2 {
    ...
    public static List<Integer> streamMax(int[] nums){
        ...
        for(int num: nums){
            heap.insert(num);
            output.add(heap.getNodeValue(0));
        }
