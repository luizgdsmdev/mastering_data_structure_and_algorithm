...
//Your task is to find the kth smallest element in the array nums. In other words, if the elements
//of the array were sorted in ascending order, the element at the kth position (1-indexed) is the output.
//
...
//The second parameter is an integer, k (1 ≤ k ≤ nums.length). This is the position of the element
//to return if the array was sorted in ascending order.
//
//Output:
...
//use its methods to add elements to the heap (insert), remove the maximum element (remove), and get the
//current size of the heap (getHeap().size()).
...
//Note: You should consider whether your solution handles edge cases, such as an array of length 1
//or when all elements in the array are the same.
...
public class exerc_1 {
    ...
    public static int findKthSmallest(int[] nums, int k){
//      Approach using build-in priority Queue
        PriorityQueue<Integer> newHeap = new PriorityQueue<Integer>(Comparator.reverseOrder());
        ...
        return newHeap.remove();

//      Solution using the min-value inversion
//        Heap newHeap = new Heap();
//
//        for(int num: nums){
//            newHeap.insertMin(num);
//        }
//        System.out.println(newHeap.getHeap());
//
//        return newHeap.getNodeValue(k - 1);

//      Solution using the max-value inversion
//      Heap newHeap = new Heap();
//
//      for(int num: nums){
//          newHeap.insert(num);
//      }
//
//      return newHeap.getNodeValue(newHeap.getSize() - (k - 1));
    }
