...
// -> Runtime: 8ms, beats 97.66% of submissions.
// -> Memory Usage: 46.24MB, beats 20.53% of submissions.
...
public class Fair_Candy_Swap {
    public static int[] fairCandySwap(int[] aliceSizes, int[] bobSizes) {
        if(aliceSizes.length == 1 && bobSizes.length == 1) return new int[]{aliceSizes[0], bobSizes[0]};

        Set<Integer> bobSet = new HashSet<>();
        int alice = 0;
        int bob = 0;

        for(int candy: aliceSizes) alice += candy;
        ...
        for(int candy: aliceSizes){
            int value = candy + diff;
            if(bobSet.contains(value)) return new int[]{candy, value};
        }
