...
// -> Runtime: 12ms, beats 70.37% of submissions.
// -> Memory Usage: 44.59MB, beats 79.99% of submissions.
...
public class Unique_Email_Addresses {
    public static int numUniqueEmails(String[] emails) {
        ...
        for(String email: emails){
            ...
            String domainName = email.substring(middle);

            for(int i = 0; i < middle; i++){
                if(email.charAt(i) == '+') break;;
                if(email.charAt(i) != '.') localName.append(email.charAt(i));
            }

            set.add(localName.toString() + domainName);
        }
